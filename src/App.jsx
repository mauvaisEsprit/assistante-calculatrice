...
function App() {
  ...
  const handleIndemnityRateChange = (e) => {
    ...
    if (value === "" || (/^\d*\.?\d{0,2}$/.test(value) && !isNaN(value))) {
  ...
  const handleDeleteWorkDay = (indexToDelete) => {
    const confirmed = window.confirm(
      "Êtes-vous sûr de vouloir supprimer cette journée ?"
    );
    if (confirmed) {
      const newWorkData = workData.filter(
  ...
  const generatePDF = async () => {
    const sortedWorkData = [...workData].sort(
    ...
    tableHeader.forEach((header, i) => {
      doc.text(
        header,
        20 + columnWidths.slice(0, i).reduce((a, b) => a + b, 0) + 2,
        yPosition + 7
      );
    });
    yPosition += 10;
    ...
    // Подсчёт
    const { normalHours, overtimeHours } = totals;
    ...
    const totalIndemnity =
      uniqueDates.size * (isNaN(indemnityRate) ? 0 : indemnityRate);
    ...
    yPosition += 10;
    doc.text(
      `Heures supplémentaires : ${overtimeHours.toFixed(2)} h`,
      20,
      yPosition
    );
    yPosition += 10;
    ...
    yPosition += 10;
    doc.text(
      `Paiement heures normales : ${normalPay.toFixed(2)} €`,
      20,
      yPosition
    );
    yPosition += 10;
    doc.text(
      `Paiement heures supplémentaires : ${overtimePay.toFixed(2)} €`,
      20,
      yPosition
    );
    yPosition += 10;
    doc.text(
      `Indemnités d'entretien : ${totalIndemnity.toFixed(2)} €`,
      20,
      yPosition
    );
    yPosition += 10;
  ...
  const getWeekNumber = (date) => {
    ...
    const daysInBetween = Math.floor(
      (tempDate - firstDayOfYear) / (24 * 60 * 60 * 1000)
    );
  ...
  return (
    <div className="App">
      ...
      <div className="form">
        ...
        <label>Indemnité d'entretien: </label>
        <input
          type="number"
          step="0.01"
          value={indemnityRate}
        ...
        <button className="add-button" onClick={handleAddWorkDay}>
      ...
      <div className="workHistory">
        ...
        <ol>
          {[...workData]
            .sort((b, a) => new Date(b.date) - new Date(a.date)) // сортировка от новой к старой
            .map((day, index) => (
              <li key={index}>
                {`${new Date(day.date).toLocaleDateString("fr-FR")} - ${
                  day.startTime
                } à ${day.endTime} - ${day.workedHours.toFixed(2)} h`}
                <button
                  className="butt-supp"
        ...
        <div className="totals">
          ...
          <p>
            Normales:{" "}
            <span className="value">{totals.normalHours.toFixed(2)} h</span>
          </p>
          <p>
            Supplémentaires:{" "}
            <span className="value">{totals.overtimeHours.toFixed(2)} h</span>
          </p>
          <h2>
            Paye total:{" "}
            <span className="value">
              {(
                totals.normalHours * hourlyRate +
                totals.overtimeHours * hourlyRate * overtimeMultiplier +
          ...
          <p>
            Paye normale:{" "}
            <span className="value">
              {(totals.normalHours * hourlyRate).toFixed(2)} €
            </span>
          </p>
          <p>
            Paye supplémentaire:{" "}
            <span className="value">
              {(totals.overtimeHours * hourlyRate * overtimeMultiplier).toFixed(
                2
              )}{" "}
              €
            </span>
